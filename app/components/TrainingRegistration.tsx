...
const COMMUNITIES=['EXÉRCITO','STAFF','BFE','CIE','BAC','BPE']as const;
const TRAININGS:Record<(typeof COMMUNITIES)[number],string[]>={
  'EXÉRCITO':['Tiro ao alvo','Navegação terrestre','Primeiros socorros','Combate corpo a corpo','Instruções de rádio','Marcha de resistência'],
  'STAFF':['Moderação','Suporte ao membro','Relatórios','Gestão'],
...
type Props={instructor:string;preview?:boolean};

export function TrainingRegistration({instructor,preview=false}:Props){
  const[community,setCommunity]=useState<(typeof COMMUNITIES)[number]>('EXÉRCITO');
  const[training,setTraining]=useState(TRAININGS['EXÉRCITO'][0]);
  ...
  const options=TRAININGS[community];
  const canSubmit=participants.length>0&&!!training&&observation.trim().length>=10&&!!proof&&status!=='sending';
  ...
  function changeCommunity(next:(typeof COMMUNITIES)[number]){
    setCommunity(next);setTraining(TRAININGS[next][0]);setError('');setStatus('editing');
  }
  ...
  async function submit(event:FormEvent){
    ...
    try{
      ...
      body.set('community',community);body.set('training',training);body.set('participants',JSON.stringify(participants));
  ...
  return<form className="training-workflow" onSubmit={submit}>
    ...
    <div className="training-layout"><section className="panel training-form-panel">
      <div className="form-section"><div className="form-section-head"><span>01</span><div><b>Comunidade e instrução</b><small>Selecione onde o treinamento foi realizado.</small></div></div><div className="community-picker">{COMMUNITIES.map(item=><button type="button" key={item} className={community===item?'active':''} onClick={()=>changeCommunity(item)}>{item}</button>)}</div><label className="field-label" htmlFor="training-type">Tipo de treinamento</label><select id="training-type" value={training} onChange={event=>setTraining(event.target.value)}>{options.map(item=><option key={item}>{item}</option>)}</select></div>
    ...
    </section><aside className="panel training-summary"><span className="kicker">CONFERÊNCIA</span><h2>Resumo do registro</h2><dl><div><dt>Instrutor</dt><dd>{instructor}</dd></div><div><dt>Comunidade</dt><dd>{community}</dd></div><div><dt>Treinamento</dt><dd>{training}</dd></div><div><dt>Participantes</dt><dd>{summary}</dd></div><div><dt>Prova da cunha</dt><dd className={proof?'ready':''}>{proof?'Pronta':'Pendente'}</dd></div></dl><div className="discord-route"><span>DC</span><div><b>Destino automático</b><small>{preview?'Discord simulado no preview':'Canal de treinamentos do Discord'}</small></div></div><p className="proof-guidance"><b>A foto precisa comprovar o treino.</b> Confirme a formação em cunha, os jogadores visíveis e a bandeira brasileira ao fundo antes de enviar.</p></aside></div>
  </form>;
}
