...
type Member={userId:string;username:string;displayName:string;rankName:string;division:string;divisions:string[];avatar:string;roleId:string;rankNumber:number;cdpActive:boolean;cdpStartedAt:string|null;cdpEndsAt:string|null;cdpRecordId?:string|null};
...
export default function Home(){
  ...
  const pages:Record<string,()=>React.ReactElement>={
    ...
    'Militares':()=><MilitaresView user={user}/>,
    ...
    'Painel do criador':()=><PainelCriadorView user={user}/>,
...
{selected?<aside className="panel member-detail roster-detail"><div className="member-header"><span className="member-avatar avatar-fallback">{selected.username.slice(0,2).toUpperCase()}{selected.avatar&&<img src={selected.avatar} alt="" onError={e=>{(e.currentTarget as HTMLImageElement).style.display='none'}}/>}</span><div><b>{selected.username}</b>{selected.displayName&&selected.displayName!==selected.username&&<small>{selected.displayName}</small>}<small>{selected.rankName}</small><small>Comunidades: {selected.division}</small><small>ID: {selected.userId}</small></div></div><CdpMemberControl key={selected.userId} member={selected} canManage={Boolean(user.isAdmin||user.isCreator||user.isHighCommand)} onChange={updateCdp}/><a className="member-profile-link" href={`https://www.roblox.com/users/${selected.userId}/profile`} target="_blank" rel="noreferrer">Abrir perfil no Roblox ↗</a></aside>:<aside className="panel help roster-help"><span className="kicker">COMUNIDADES CONECTADAS</span><h2>Selecione um integrante</h2><p>Clique em um registro para conferir patente, comunidades e situação da CDP.</p><div className="community-summary">{DIVISION_SIGLAS.map(sigla=><div key={sigla}><span>{sigla}</span><b>{counts[sigla]??'—'}</b></div>)}</div><div className="initial-cdp-note"><i/>Todos começam fora da CDP</div></aside>}</div>}
...
function CapacitacaoView({user}:{user:RobloxUser}){
  return<CdpPersonalStatus user={user}/>}
...
function PainelCriadorView({user}:{user:RobloxUser}){
  const[tab,setTab]=useState<'cdp'|'patentes'|'massa'|'treinos'>('cdp');
  if(!user.isCreator)return<div className="workspace-grid"><article className="panel workspace"><EmptyState title="Acesso restrito" text="A gestão geral de patentes é exclusiva do Criador."/></article></div>;
...
{tab==='massa'&&<BulkRankManager/>}
{tab==='treinos'&&<TreinosConfigView/>}
</article></div>}

function CdpConfigView(){return<CdpSettingsManager/>}
